...
async def wait_for_highcharts(page, timeout=30):
    """Poll until Highcharts has rendered at least one chart."""
    for i in range(timeout):
        ready = await page.evaluate("""
            () => {
                return (typeof Highcharts !== 'undefined' &&
                        Highcharts.charts &&
                        Highcharts.charts.filter(c => c).length > 0);
            }
        """)
        if ready:
...
async def get_wind_data():
    ...
    async with async_playwright() as p:
        ...
        print("  Page loaded, waiting for Highcharts...", flush=True)

        if not await wait_for_highcharts(page):
            print("  ERROR: Highcharts did not load in time", flush=True)
            await browser.close()
            return []
