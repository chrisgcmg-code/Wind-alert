...
Loads bigwavedave.ca once, sets datepicker + calls getMet(1) for each day.
getMet(1) uses the 'initial' path which doesn't access chart.series directly.
"""
    ...
    async with async_playwright() as p:
        ...
        for day_str in days:
            ...
            if current_dp != day_str:
                print(f"  Setting date to {day_str}...", flush=True)
                await page.evaluate(f"document.getElementById('datepicker').value = '{day_str}'")
                await page.evaluate("getMet(1)")
                await page.wait_for_timeout(8000)
            else:
                print(f"  Already on {day_str}, waiting for chart...", flush=True)
